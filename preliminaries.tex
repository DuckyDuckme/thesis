...
There are many bases for a lattice (actually, for $n \geq 2$i, there are infinitely many as can be proven using a diagonalization argument), some "better" than others. This will be the foundation for some of the problems like \prob{SVP} or \prob{CVP}.
...
In general, a \textit{number field} is defined as a subfield of $\oQ$ having finite degree (the dimension as a vector space) over the rationals $\Q$. The \textit{degree} of a number field is defined as the degree of the monic minimal polynomial\footnote{Recall that we call polynomial monic if its leading coefficient is 1. It is the minimal polynomial for some element $\alpha$ if it is a polynomial of the lowest degree such that $\alpha$ is its root.} of the adjoint root. Throughout this section, we fix the imaginary numbers $\oQ = \C$ as an algebraic closure of the rationals.
...
\end{definition}
In fact, \textit{algebraic integers} form a ring.
...
\begin{example}
    The ring of Gaussian integers, defined as $K = \Q(\sqrt{-1}) = \{a + b\sqrt{-1}\, :\, a,b\in \Q\}$
    has degree 2 since $x^2+1$ is the minimal polynomial of $\sqrt{-1}$ over $\Q$ and $\Oo_K = \Z[\sqrt{-1}]$. 
\end{example}
