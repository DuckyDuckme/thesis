...
\end{definition}
	For example, in order to be able to go ``away'' from the lattice $\Ll$ ``out'' into the ambient space $\R^n$ and still be able to return to the original vector, we need to make sure the distance $d$ we travel is at most $\lambda_1(\Ll)/2$.
...
\[ \Phi_n(x) = \]
It can be shown that every such $\zeta$ is equal to precisely $e^{2\pi \sqrt{-1}/n}$ and $\zeta^i = \zeta^j$ if and only if $i = j$.
