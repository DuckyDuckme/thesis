...
However, this comfortable security comes at great cost because, as shown in \cite{int_scheme}, the parameters chosen to assure the secrecy, yield a scheme that has complexity of $\tilde{O}(\lambda^{10})$ where $\lambda$ is our security parameter (the greater it is the more secure message). As a small example, consider $\lambda = 10$ as the (small) key size. To now encrypt a single(!) bit, it will take approximately $10^{10}$ operations. On a modern laptop this would take a little less than 5 seconds. To send the message 'hello', we need to use 5 letters $*$ 16-bits per letter $= 5*16*5 = 650$ seconds which is more than 10 minutes! As one can imagine, this is completely impractical for most applications. It is now mainly used as a proof of concept and base for further developments in FHE area.
