...
On top of that, following the definition given in \cite{lbc}, we define the \textit{statistical distance} as follows.
...
\begin{definition}[Dual]
	...
	\[ \Lld = \{ y \in \R^n : \langle y,\Ll \rangle \subset \Z \}.\]
\end{definition}
...
\begin{example}\label{ld-ex}
    Take $\mathcal{L}= \Z 
		...
		\Z \big(\begin{smallmatrix} 0\\ 1 \end{smallmatrix}\big)$. See Figure \ref{lattices}.
...
One of the most important concepts invoked in the paper is the so called \textit{smoothing parameter} $\eta$ of a lattice $\Ll$. We define it as follows.
...
\end{definition}
In other words, $\eta_{\epsilon}(\Ll)$ is the smallest $s$ such that a Gaussian measure scaled by $1/s$ on the dual lattice $\Lld$ gives all but $\epsilon/(1+\epsilon)$ of its weight to the origin - \cite{regev}. As the name suggests, $\epsilon$ is usually taken to be very small and in fact, a negligible function of $n$. Roughly speaking, it provides the width beyond which the discrete Gaussian measure on a lattice behaves like a contiuous one. On the Figure \ref{smoothing} there are plotted two Gaussians. The first one has very small deviation and assigns most of its values to the origin. The second one, with the deviation $\sqrt{10}$ times the size of the first one, looks much more \textit{smooth} and continuous. For an appropriate choice of $\epsilon$, the smoothing parameter will lay somewhere between those two values.

\begin{figure}
        \caption{2 dimensional Gaussian with standard deviation $r$}
        \label{smoothing}
        \centering
        \begin{subfigure}{.5\textwidth} 
                \centering
