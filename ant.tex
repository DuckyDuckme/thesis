...
As mentioned before, it is not in general true that $\Oo_K$ is a principal ideal domain. To mimic the notion of divisibility we will associate the elements of $\Oo_K$ with fractional ideals.
\begin{definition}[Fractional ideal]
	We say that $\I$ is \textit{fractional}, when there exists an element $\alpha \in \Oo_K$, $\alpha \neq 0$, such that $\alpha \cdot \I \subseteq \Oo_K$.
\end{definition}
The set of nonzero fractional ideals of $\Oo_K$ forms a group under multiplication with multiplication defined as $(\alpha \I)(\beta \J) := \alpha \beta \, \I \J$ and $\Oo_K$ iteself acting as the identity element. %Since any (fractional) ideal $\I$ factorizes uniquely into prime ideals
